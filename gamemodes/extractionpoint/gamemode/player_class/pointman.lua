...
PLAYER.DisplayName			= "POINTMAN"

PLAYER.ClassDescription 	= "POINTMAN, WEAPON SPECIALIST\nIMPROVED ACCURACY\nBONUS PRIMARY WEAPONS"
PLAYER.ClassPros			= {	"NO PENALTY FOR AIMING",
								"NO PENALTY FOR HIPFIRE",
								"BONUS PRIMARY",}
PLAYER.ClassCons			= { }
					
PLAYER.ClassTeam			= 2
PLAYER.Model				= "models/player/barney.mdl" --models/player/phoenix.mdl

local pointman_bonus = {	"weapon_ep_mp5",
							"weapon_ep_combat",
							"weapon_ep_mg",
							"weapon_ep_shotgun",
							"weapon_ep_rifle"}
...
function PLAYER:GiveGuns()	
	...
	if self.Player:Team() == TEAM_SWAT then
		local primary = table.Random(swat_primary)
		local pointman_culled = {} --Table to cull weapons pointman is holding.
		for k,v in pairs(pointman_bonus) do
			if !(v == primary) then
				pointman_culled[k] = v
			end
		end
		self:GiveWeapon(primary) -- Primary
		self:GiveWeapon(table.Random(pointman_culled)) -- Bonus pointman Primary
...
player_manager.RegisterClass( "player_pointman", PLAYER, "player_default" )