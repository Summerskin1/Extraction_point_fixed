...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( vPos )
		...
		for i=0, 5 do
			...
			if (particle) then
				local mag = math.Rand(0,-200)
				particle:SetVelocity( dir*mag+(AngleRand():Forward())*mag+dir*10 )
				particle:SetLifeTime( 0 )
				particle:SetDieTime( math.Rand( 2, 3 ) )
				particle:SetStartAlpha( math.Rand( 200, 255 ) )
				particle:SetEndAlpha( 0 )
...
function EFFECT:Render()
	...
	render.DrawBeam( self.StartPosition, self.EndPosition, (1+CurTime()-self.Start)*18, 0, 0, Color( 255,255,0, math.Clamp(20*self.Thick,0,255) ) )
	render.DrawBeam( self.StartPosition, self.EndPosition, (1+CurTime()-self.Start)*5, 0, 0, Color( 255,255,255, math.Clamp(20*self.Thick,0,255) ) )
end
