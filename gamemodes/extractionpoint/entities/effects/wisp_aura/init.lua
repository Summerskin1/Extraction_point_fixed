...
function EFFECT:Think()
	...
	if self.Owner:IsPlayer() && !self.Owner:Alive() then
		self.Emitter:Finish()
		return false	
	end
	...
	if self.LastEmit < CurTime() then
		self.LastEmit = CurTime()+0.04
			local particle = self.Emitter:Add( "particle/smokesprites_0006", self.Owner:GetPos()+Vector(0,0,40) ) 
			 local mag = math.Rand(2,3)
			 local vel = (AngleRand():Forward() * 30 * mag)
			 vel.z = vel.z/2
			...
			particle:SetStartSize( (10-mag)*2 ) 
			particle:SetEndSize( (10-mag)*5 ) 
			particle:SetColor( 250,220,150 ) 
			particle:SetAirResistance( 150 )
			particle:SetGravity( Vector(math.Rand(-200,200),math.Rand(-200,200),math.Rand(-200,200)))
			particle:SetCollide( true )
			for i=0,5 do
				local particle = self.Emitter:Add( "sprites/light_glow02_add", self.Owner:GetPos()+Vector(0,0,40) ) 
				 local mag = math.Rand(2,3)
