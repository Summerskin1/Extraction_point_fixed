...
function ENT:Initialize()
	...
	ed:SetScale( 1 )
	util.Effect( "wisp_charge", ed, true, true )
...
function ENT:Think()
	if SERVER then
		if (self:GetLife() <= 0 and !self:GetDead()) then
			...
			ed:SetOrigin(self.Entity:GetPos())
			ed:SetScale( 1 )
			util.Effect( "wisp_explode", ed, true, true )
			...
			local targets = ents.FindInSphere(self:GetPos(),550)
				...
				if v:IsValid() and v:IsPlayer() then
					if SERVER then
						...
						if (traceRes1.Entity == v or traceRes2.Entity == v) then 
							v:TakeDamage( 50, owner, owner )
						end
					end
				end
			end
			util.BlastDamage(owner, owner, self:GetPos(), 550, 25)
